package tender.handler.recalc_end_rent_date
...
class Trigger extends _DoScheduledHandler {
    ...
    public int doHandler(_Session session) {
        try {
            ...
            def coll = db.getCollectionOfDocuments("form='orderleasing'", true).getEntries()
            def doc = null;
            Date end_rent = null;
            ...
            coll.each {
                doc = it.getDocument();
                if (doc) {
                    end_rent = new SimpleDateFormat("dd.MM.yyyy").parse(doc.getValueString("endrentdate"));
                    println(end_rent)
                    if (end_rent) {
