...
class QueryOpen extends _FormQueryOpen {
	...
	public void doQueryOpen(_Session ses, _WebFormData webFormData, String lang) {
		...
		publishValue("title",getLocalizedWord("Недвижимое имущество - Здание", lang))
	...
	public void doQueryOpen(_Session ses, _Document doc, _WebFormData webFormData, String lang) {
		...
		actionBar.addAction(new _Action(getLocalizedWord("Закрыть",lang),getLocalizedWord("Закрыть без сохранения",lang),_ActionType.CLOSE))
		if(doc.getValueString("isrented") !='1' && user.getUserID() != "user"){
            if(doc.doc.getViewTextList()[5] == "tender") {
                actionBar.addAction(new _Action("Подать заявку на участие в тендере", "Подать заявку на участие в тендере", "SUBMIT_APPLICATION_FOR_TENDER"))
                publishValue("tenderid", cdb.getDocumentByID(doc.getValueString("tender")).getDocID())
            }
            else
			    actionBar.addAction(new _Action("Подать заявление на аренду", "Добавить заявление на аренду", "SUBMIT_APPLICATION_FOR_RENT"))
		}
		publishElement(actionBar)
		...
		publishValue("rent_cost", doc.getValueString("rent_cost"))
        publishValue("rent_comment", doc.getValueString("rent_comment"))
