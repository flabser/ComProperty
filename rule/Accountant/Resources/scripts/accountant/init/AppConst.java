package accountant.init;

public class AppConst {
	public static String NAME = "Accountant";
	public static String NAME_ENG = "Accountant";
	public static String NAME_RUS = "Импорт объектов";
	public static String NAME_KZ = "Нысан импорт";
	public static String DEFAULT_URL = "p?id=upload-updating-form";
	public static String FT_INDEX_SCOPE = "[{\"tableName\":\"properties\",\"fieldNames\":"
	        + "[\"object_name\",\"description\",\"notes\",\"inv_number\"],\"daoImpl\":\"municipalproperty.dao.PropertyDAO\"}]";
}
