package form.leasing_rentapplication
...
class PostSave extends _FormPostSave {
    ...
    void doPostSave(_Session ses, _Document doc) {
        ...
        println("Hello postsave")
        def formData = new _WebFormData();
        formData.setSign(doc.getParentDocumentID());
        def nav = ses.getPage("open_tender", formData)
        publishElement(nav)
