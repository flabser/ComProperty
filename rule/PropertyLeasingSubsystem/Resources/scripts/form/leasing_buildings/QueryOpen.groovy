package form.leasing_buildings
...
class QueryOpen extends _FormQueryOpen {
	...
	public void doQueryOpen(_Session ses, _Document doc, _WebFormData webFormData, String lang) {
		...
		if(ses.db.getGlossaryCustomFieldValueByDOCID(doc.getValueString("propertycode").toInteger(), "code") == '014'){
			actionBar.addAction(new _Action("Добавить договор аренды", "Добавить договор", "ADD_LEASING_CONTRACT"))
		}else{
			actionBar.addAction(new _Action("Объявить в аренду", "Объявить в аренду", "ADD_LEASING"))
		}
		actionBar.addAction(new _Action("Добавить помещение", "Добавить помещение", "ADD_ROOMS"))
		actionBar.addAction(new _Action(getLocalizedWord("Закрыть",lang),getLocalizedWord("Закрыть без сохранения",lang),_ActionType.CLOSE))
		if(doc.getValueString("isrented") !='1'){
