 ...
 * 21ый лист
 */
class DoScript21 extends _DoScript {
	...
	public void doProcess(_Session session, _WebFormData webFormData, String lang) {
		...
		// Листы
		Sheet sheet1 = excelFile.getSheet(21);
		
		// for(int i=7; i<=17; i++) легковые автомашины
		// for(int i=18; i<=49; i++) Грузовые cargo
		// specialequipment
		
		for(int i=456; i<529; i++){
			// col - column
			 
			Cell model = sheet1.getCell(1, i);
			
			println(sheet1.getCell(12, i).getContents())
						
			def doc = new _Document(session.getCurrentDatabase());
			doc.addStringField("model", model.getContents())
			doc.addStringField("yearrelease", sheet1.getCell(4, i).getContents())
			doc.addStringField("bodynumber", sheet1.getCell(11, i).getContents())
			doc.addStringField("enginenumber", sheet1.getCell(7, i).getContents())
			doc.addStringField("originalcost", sheet1.getCell(12, i).getContents())
			doc.addStringField("residualcost", sheet1.getCell(13, i).getContents())
			doc.addStringField("namerus", "Тазалык" )
			doc.addStringField("invnumber", sheet1.getCell(6, i).getContents())
