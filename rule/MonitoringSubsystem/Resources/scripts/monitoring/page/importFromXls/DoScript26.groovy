 ...
 * 26ый лист
 */
class DoScript26 extends _DoScript {
	...
	public void doProcess(_Session session, _WebFormData webFormData, String lang) {
		...
		// Листы
		Sheet sheet1 = excelFile.getSheet(26);
		
		// for(int i=9; i<=495; i++) hospitaltransport
		//for(int i=498; i<=505; i++){   cargo
		// for(int i=508; i<=509; i++){ bus
		// 
		 
		// specialequipment
		for(int i=514; i<=539; i++){
			// col - column
			...
			doc.addStringField("originalcost", sheet1.getCell(10, i).getContents())
			doc.addStringField("residualcost", sheet1.getCell(11, i).getContents())
			doc.addStringField("namerus", "Спец АТП" )
			doc.addStringField("invnumber", sheet1.getCell(5, i).getContents())
