...
class individuals extends _DoScript { 
	...
	public void doProcess(_Session session, _WebFormData formData, String lang) {
		...
		def user = session.getCurrentAppUser();
			if (user.hasRole(["registrator","system_administrator"])){
				def newDocAction = new _Action(getLocalizedWord("Добавить", lang),getLocalizedWord("Добавить физическое лицо",lang), "new_document")
				newDocAction.setURL("Provider?type=edit&id=individuals&key=")
