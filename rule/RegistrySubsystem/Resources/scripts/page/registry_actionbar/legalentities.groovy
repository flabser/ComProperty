	...
	public void doProcess(_Session session, _WebFormData formData, String lang) {
		...
		def user = session.getCurrentAppUser();
			if (user.hasRole(["registrator","system_administrator"])){
				def newDocAction = new _Action(getLocalizedWord("Добавить", lang),getLocalizedWord("Добавить юридическое лицо",lang), "new_document")
				newDocAction.setURL("Provider?type=edit&id=legalentities&key=")
