...
class Trigger extends _DoScheduledHandler {
    ...
    public int doHandler(_Session session) {
        ...
        File xlsFile = new File("Сооружения.xls");
        ...
        for (int i = 1; i < sheet.getRows(); i++) {
                 ...
                 if (cell.type != CellType.EMPTY && cell.cellFormat != null) {
                     switch (j) {
                         ...
                         case 25:
                             ...
                             break
                         case 26:
                             String kt = ((LabelCell) cell).getString().trim()
                             def kt_glos = db.getGlossaryDocument("kt", "viewtext='" + kt + "'");
                             if (!kt_glos) {
                                 kt_glos = new _Glossary(db)
                                 kt_glos.setForm("kt");
                                 kt_glos.setValueString("form", "kt");
                                 kt_glos.setValueString("name", kt)
                                 kt_glos.setViewText(kt)
                                 kt_glos.addViewText(kt)
                                 kt_glos.save("[supervisor]");
                             }
                             doc.addGlossaryField("kt", kt_glos.docID)
                             break;
                         case 27:
                             ...
                             break
                         case 42:
                             String description = ((LabelCell) cell).getString()
                             doc.setValueString("description", description)
                             break
                         default:
                             break
                     }
            ...
            doc.addGlossaryField("material", 0)
