	...
	public void doProcess(_Session session, _WebFormData formData, String lang) {
		...
		def user = session.getCurrentAppUser();
			if (user.hasRole(["registrator","system_administrator"])){
				def newDocAction = new _Action(getLocalizedWord("Добавить МРП", lang),getLocalizedWord("Добавить МРП",lang), "new_glossary")
				newDocAction.setURL("Provider?type=edit&id=mrp&key=")
