	...
	public void doProcess(_Session session, _WebFormData formData, String lang) {
		...
		def user = session.getCurrentAppUser();
			if (user.hasRole(["registrator","system_administrator"])){
				def newDocAction = new _Action(getLocalizedWord("Новый ОКЭД", lang),getLocalizedWord("Добавить ОКЭД",lang), "new_glossary")
				newDocAction.setURL("Provider?type=edit&id=oked&key=")
