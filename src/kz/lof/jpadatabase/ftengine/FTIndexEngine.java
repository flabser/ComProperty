...
public class FTIndexEngine implements IFTIndexEngine, Const {
	...
	public String normalizeKeywordToTSQuery(String keyword){
		//String _keyWord = keyword.trim().replaceAll("\\s+{2}", " ").replace(" ", " & ").replace(":", "\\:");

		return keyword;
	}

	@Override
	public _ViewEntryCollection search(String keyWord, User user, int pageNum,int pageSize, String[] filters, String[] sorting) throws FTIndexEngineException{
        ...
        try {
            ...
            String sql = "SELECT distinct " + fields + " FROM maindocs, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                  ...
                  /*  " union \n" +
                    ...
                    " union \n" +
                    "SELECT distinct " + fields + " FROM executions, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                    (filterCondition.length() != 0 ? " and " + filterCondition : "" ) +
                    " and docid in (select docid from readers_executions where username  IN (" + cuID + "))" +
                    " union \n" +
...
/*            sql = "SELECT SUM(count) from (SELECT count(distinct docid), 'maindocs' as tablename FROM maindocs, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                    ...
                    " union \n" +
                    "SELECT count(distinct docid), 'custom_fields' as tablename FROM custom_fields, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                    " and docid in (select docid from readers_maindocs where username IN (" + cuID + ")) and \n" +
                    "  docid not in (SELECT distinct docid FROM maindocs, plainto_tsquery('" + _keyWord + "') q where fts @@ q)" +
                    " union \n" +
                    "SELECT count(distinct docid), 'tasks' as tablename FROM tasks, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                    " and docid in (select docid from readers_tasks where username IN (" + cuID + "))" +
                    " union \n" +
                    "SELECT count(distinct docid), 'executions' as tablename FROM executions, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                    " and docid in (select docid from readers_executions where username IN (" + cuID + "))" +
                    " union \n" +
                    "SELECT count(distinct docid), 'projects' as tablename FROM projects, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
                    " and docid in (select docid from readers_projects where username IN (" + cuID + "))) as count;";*/
		...
		if (pageNum == 0)pageNum = RuntimeObjUtil.countMaxPage(coll.getCount(), pageSize);	
	...
	@Deprecated
	public StringBuffer ftSearch(Set<String> complexUserID, String absoluteUserID,  String keyWord, int offset, int pageSize) throws DocumentException, FTIndexEngineException, ComplexObjectException {
	...
	public int ftSearchCount(Set<String> complexUserID, String absoluteUserID, String keyWord) throws DocumentException {
		...
		try {
			...
			String sql = "SELECT count(distinct docid), 'maindocs' as tablename FROM maindocs, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
					" and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"')" + 
					" union \n" +
					"SELECT count(distinct docid), 'custom_fields' as tablename FROM custom_fields, plainto_tsquery('" + _keyWord + "') q where fts @@ q \n" +
					" and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"') and \n" +
			...
			/*String sql = "SELECT distinct docid, 'maindocs' as tablename FROM maindocs, to_tsquery('" + _keyWord + "') q where fts @@ q \n" +
            " and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT id as docid, 'custom_fields' as tablename FROM custom_fields, to_tsquery('" + _keyWord + "') q where fts @@ q \n" +
            " and docid in (select docid from readers_maindocs where username = '"+ absoluteUserID +"' and " +
            "  docid not in (SELECT distinct docid FROM maindocs, to_tsquery('" + _keyWord + "') q where fts @@ q))" + 
            " union \n" +
            "SELECT distinct docid, 'tasks' as tablename FROM tasks, to_tsquery('" + _keyWord + "') q where fts @@ q \n" +
            " and docid in (select docid from readers_tasks where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT distinct docid, 'executions' as tablename FROM executions, to_tsquery('" + _keyWord + "') q where fts @@ q \n" +
            " and docid in (select docid from readers_executions where username = '"+ absoluteUserID +"')" + 
            " union \n" +
            "SELECT distinct docid, 'projects' as tablename FROM projects, to_tsquery('" + _keyWord + "') q where fts @@ q \n" +
            " and docid in (select docid from readers_projects where username = '"+ absoluteUserID +"');";*/
			...
			/*while (rs.next()) {
                ...
                if (table.equalsIgnoreCase("custom_fields")){
                    ...
                    if (rs1.next()) {
                        ...
                        try {
                            doc = db.getMainDocumentByID(docID, complexUserID, absoluteUserID);
                            count ++ ;
                        } catch (DocumentAccessException e) {
