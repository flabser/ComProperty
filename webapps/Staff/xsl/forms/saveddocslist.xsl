...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<html>
			<head>
				<title>
					<xsl:value-of select="$doctype"/> - 4ms workflow
				</title>
								...
								<form action="Provider" name="frm" method="post" id="frm" enctype="application/x-www-form-urlencoded">
									<div display="block"  id="property">
										...
										<table width="80%" border="0" >
											<tr>
												<td class="fc">Имя файла :</td>
									            <td>
					                                 <input type="text" name="filename" value="{document/fields/filename}" size="59" class="td_noteditable" style="width:350px">
															<xsl:attribute name="readonly">readonly</xsl:attribute>
					                                 </input>
					                           </td>   					
											</tr>
											<tr>
												<td class="fc">Загрузил :</td>
									            <td>
					                                 <input type="text" name="author" value="{document/fields/author}" size="59" class="td_noteditable" style="width:350px">
															<xsl:attribute name="readonly">readonly</xsl:attribute>
					                                 </input>
					                           </td>
											</tr>
									      	<tr>
												<td class="fc">Номера колонок :</td>
								            	<td>
					                                 <textarea class="textarea_editable" rows="4" name="description" value="{document/fields/cols}" cols="45" style="width:600px">
					                                 	<xsl:if test="$editmode != 'edit'">
															<xsl:attribute name="readonly">readonly</xsl:attribute>
															<xsl:attribute name="class">textarea_noteditable</xsl:attribute>
														</xsl:if>
					                                 	<xsl:value-of select="document/fields/cols"/>
					                                 </textarea>
					                           </td>   					
											</tr> 
									</table>
