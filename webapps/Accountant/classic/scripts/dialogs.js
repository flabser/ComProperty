...
function pickListSingleOk(docid){
	...
	newTable="<table id="+ queryOpt.tablename +"><tr><td style='width:600px;' class='td_editable'>"+ text 
	if (queryOpt.fieldname == 'corr' || queryOpt.fieldname == 'recipient' && queryOpt.queryname == 'corrcat'  ){
		newTable+="<span style='float:right; border-left:1px solid #ccc; width:20px; padding-right:10px; padding-left:2px; padding-top:1px; color:#ccc; font-size:11px'>"+docid+"</span>"
	}
...
function removeCoordinator(docid,el){
	text=$("#"+docid).attr("value");
	$(el).remove();
	tr="<div style='display:block; width:100%; text-align:left; cursor:pointer' name='itemStruct' onClick='selectItem(this)' " +
		"ondblclick='addCoordinator(&quot;"+docid+"&quot;,this)'><input class='chbox' type='hidden' name='chbox' id='"+ docid+"' value='"+ text +"'></input>" +
		"<font style='font-size:12px'>"+ text+"</font></div>";
	...
	$.ajax({
		...
		success:function (data){
			if (data.match("login") && data.match("password")){
				...
				if(queryOpt.fieldname == 'executor' || queryOpt.fieldname == 'taskauthor' || queryOpt.fieldname == 'intexec' || queryOpt.fieldname == 'signedby' || queryOpt.fieldname == 'signer'){
