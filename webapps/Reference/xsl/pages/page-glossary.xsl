...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"> 
	...
	<xsl:template match="page/view_content/response/content/query/entry">
		...
		<tr class="{@docid}" id="{@docid}{@doctype}" onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
			...
			<td nowrap="nowrap" style="border:1px solid #ccc">
				<div style="display:block; overflow:hidden; width:93%;">&#xA0;
					...
					<a href="{@url}" title="{@viewtext}" class="doclink" style="margin-left:2px">
						<xsl:attribute name="onclick">javascript:beforeOpenDocument()</xsl:attribute>
						<xsl:if test="@isread = '0'">
							<xsl:attribute name="style">font-weight:bold</xsl:attribute>
						</xsl:if>
	...
	<xsl:template match="entry" mode="line">
		<div class="Node" style="overflow:hidden; margin-left:10px" id="{@docid}{@doctype}">
						...
						<div id="viewcontent">
							<div id="viewcontent-header" style="height:73px;">
								...
								<div class="button_panel" style="margin-top:8px">
									<span style="float:left; padding-left:15px; margin-top:7px; margin-bottom:3px">
										<xsl:if test="//action[@id='new_glossary']/@mode = 'ON'">
											<button style="margin-right:5px; margin-top:-5px" title="{//action[@id='new_glossary']/@hint}" id="btnNewdoc">
										...
										<xsl:if test="//action[@id='delete_document']/@mode = 'ON'">
											<button id="btnDeldoc" title="{//action[@id='delete_document']/@hint}" style="margin-right:5px;margin-top:-5px">
							...
							<table class="viewtable" id="viewtable" width="100%">
								<xsl:choose>
									...
									<xsl:when test="query/@ruleid ='city'">
										...
										<xsl:for-each select ="page/view_content/response/content/query/entry">
											<tr class="{@docid}" id="{@docid}{@doctype}" onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
												<xsl:attribute name="bgcolor">#FFFFFF</xsl:attribute>
												<td style="text-align:center; border:1px solid #ccc;" width="3%">
													<input type="checkbox" name="chbox" id="{@id}" autocomplete="off" value="{@doctype}"/>
									...
									<xsl:otherwise>
									<xsl:apply-templates select="page/view_content/response/content/query/entry"/>
