...
// подтвердить регистрацию контрагента
function activateContractor( emp_form ,userid){
    enableblockform();
    $("body").notify({"text":pleasewaitactivate,"onopen":function(){}})
    $(document).unbind("keydown")
    $.ajax({
        type: "POST",
        url: "Provider?type=page&id=activate_contractor&userid=" + userid + "&empform="+emp_form,
        datatype:"xml",
        success: function(xml){
            redir = $(xml).find('redirect').text();
            if (redir == ""){
...
function sendNotification(userid){
    enableblockform()
    text=""
    divhtml ="<div id='dialog-message-comment' title='Уведомление' >";
    divhtml+="<span style='height:100px; margin-top:4%; width:100%; text-align:center'>"+
        "<font style='font-size:13px; '>"+text+"</font></span>";
    divhtml+="Текст уведомления: <br/><textarea id='message' style='width:97%; height:110px' /></div>"
    $("body").append(divhtml)
    ...
    $("#dialog-message-comment").dialog({
        ...
        buttons: {
            "Отправить": function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "Provider?type=page&id=send_notification&userid=" + userid + "&message="+ message_text,
                    datatype:"xml",
                    success: function(xml){
                        var result = $(xml).find("result").text()
                        if(result == "ok")
                            txt = "Уведомление отправлено"
                        else
                            txt = "Уведомление не отправлено"
