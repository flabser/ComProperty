	...
	<xsl:template name="header-view">	
		<div id="header-view">
			...
			<span style="float:right; padding:5px 5px 5px 0px" >
				<a id="currentuser" target="_parent" title="{columns/column[@id = 'VIEW_USERPROFILE']/@caption}" href="Provider?type=edit&amp;element=userprofile&amp;id=userprofile" style="text-decoration:none;color: #555555 ; font: 11px Tahoma; font-weight:bold">
					<font><xsl:value-of select="currentuser"/></font>
				</a>
				...
				<a target="_parent" id="helpbtn" href="/Help/Provider?type=page&amp;id=article" title="{columns/column[@id = 'HELPCAPTION']/@caption}" style="text-decoration:none;color: #555555 ; font: 11px Tahoma; font-weight:bold">
				...
				<div class="sarea" style="margin-top:15px; text-align:right; display:none">
					...
					<script type="text/javascript">    
				   		$(function() {
							$(".sarea button").button();
							$(".sarea button").css("width","30px");
							$(".sarea button").css("height","26px");
							$("#btnsearch span").css("padding","0px");
		...
		<a href="" class="actionlink">
			<xsl:choose>
				<xsl:when test="$sortfield != $namefield">
					<xsl:attribute name="href">javascript:updateView('<xsl:value-of select="@type"/>','<xsl:value-of select="@id"/>',<xsl:value-of select="query/@currentpage"/>,null,'<xsl:value-of select="$namefield"/>','ASC')</xsl:attribute>
				</xsl:when>
				<xsl:when test="$sortfield = $namefield and $sortorder = 'ASC'">
					<xsl:attribute name="href">javascript:updateView('<xsl:value-of select="@type"/>','<xsl:value-of select="@id"/>',<xsl:value-of select="query/@currentpage"/>,null,'<xsl:value-of select="$namefield"/>','DESC')</xsl:attribute>
				</xsl:when>
