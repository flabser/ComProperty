									...
									<table width="100%" border="0">
										...
										<tr>
											...
											<td>
												...
												<input type="text" value="{document/fields/yearconstruction}" style="width:40px;" class="td_editable" name="yearconstruction" autocomplete="off" pattern="^18\d\d$|^19\d\d$|^20[{substring($year,3,1)}][0-{substring($year,4,1)}]$|^200[0-9]$" onkeyup="javascript:validationyearfield(this)">
													<xsl:if test="$editmode != 'edit'">
														<xsl:attribute name="readonly">readonly</xsl:attribute>
														<xsl:attribute name="class">td_noteditable</xsl:attribute>
														<xsl:attribute name="onfocus">javascript:$(this).blur()</xsl:attribute>
