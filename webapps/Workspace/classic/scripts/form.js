...
var documentsavedcaption = "Сохранено успешно";
...
function validationfloatfield(element){
	if (/^\d+((.|,)\d+)?$/.test($(element).val()) || $(element).val().length == '0'){
		$("#btnsavedoc").removeAttr("disabled").removeClass("ui-state-disabled")
		$(element).removeClass("invalid")
	}else{
		$(element).blur().focus().addClass("invalid")
...
function validationdatefield(element){
	if (/\d{1,2}\.\d{2}\.\d{4}/.test($(element).val()) || $(element).val().length == '0'){
		$("#btnsavedoc").removeAttr("disabled").removeClass("ui-state-disabled")
		$(element).removeClass("invalid")
	}else{
		$(element).blur().focus().addClass("invalid")
...
function validationyearfield(element){
	if ((/^19\d\d$|^20[0-9][0-9]$/.test($(element).val()) && $(element).val() < new Date().getYear() + 1901) || $(element).val().length == '0'){
		$("#btnsavedoc").removeAttr("disabled").removeClass("ui-state-disabled")
		$(element).removeClass("invalid")
	}else{
		$(element).blur().focus().addClass("invalid")
...
function validationpercentfield(element){
	if (/(^(100(?:(\.|\,)0{1,2})?))|(?!^0*$)(?!^0*(\.|\,)0*$)^\d{1,2}((\.|\,)\d{1,2})?$/.test($(element).val()) || $(element).val().length == '0'){
		$("#btnsavedoc").removeAttr("disabled").removeClass("ui-state-disabled")
		$(element).removeClass("invalid")
	}else{
		$(element).blur().focus().addClass("invalid");
...
function sentNotify(docid){
	$.ajax({
		type: "get",
		url: 'Provider?type=page&id=sentnotification&docid='+docid,
		success:function (data){
...
function makePDF(){
	...
	var fields = $('form').serializeArray(),
		formData = $("form").serialize(),
		recursiveEncoded = $.param(fields);
	window.location.href = 'Provider?type=page&id=printform&'+recursiveEncoded;
	$('#loadingpage , #blockWindow').css("display","none");
	$("body").css("cursor","default");
	disableblockform();
}
...
function commentToAttachOk(){
	if ($("#commentText").val().length ==0){
		...
		if($.cookie("lang")=="ENG"){
			commentcaption ="comments"
		}else if ($.cookie("lang")=="KAZ"){
			commentcaption ="түсініктеме"
		}
		$("<tr><td></td><td style='color:#777; font-size:12px'>"+commentcaption+": "+$("#commentText").val()+"</td><td></td></tr>").insertAfter("#"+control_sum_file)
...
function usersWhichRead(el,doctype, id){
	...
	$.ajax({
		type: "get",
		url: 'Provider?type=service&operation=users_which_read&id=users_which_readdoctype='+doctype+"&key="+id,
		success:function (xml){
...
function usersWhichReadInTable(el,doctype, id){
	$.ajax({
		type: "get",
		async:true,
		url: 'Provider?type=service&operation=users_which_read&id=users_which_readdoctype='+doctype+"&key="+id+"&nocache="+Math.random() * 300,
		success:function (xml){
			$(xml).find("entry").each(function(){
...
function checkByIIN(){
    if($("#iin").val().length != 12) {
        $("#iin").focus()
        $("#iin").mouseenter()
    }  else if($("#checkButtonImg").attr("class") != "checked"){
        enableblockform()
        $("body").css("cursor","wait");
        $.ajax({
            type: "get",
            async:true,
            url: 'Provider?type=page&id=checkiin&iin='+ $("#iin").val(),
            success:function (xml){
                if($(xml).find("valid").text()=="true"){
                    $("#btnsavedoc").removeAttr("disabled").removeClass("ui-state-disabled")
                    $("#checkButtonImg").attr("class", "checked");
                    $("#checkButtonImg").attr("src", "/SharedResources/img/iconset/ok.png")
                    $("[name=fio]").val($(xml).find("fio").text())
                    $("[name=orgfullname]").val($(xml).find("org_name").text())
                    $("[name=fullactualaddress]").val($(xml).find("address").text())
                    $("[name=phone]").val($(xml).find("phone").text())
                } else{
                    infoDialog("Ошибка при проверке ИИН/БИН")
                }
...
function checkByBIN(){
    if($("#bin").val().length != 12) {
        $("#bin").focus()
        $("#bin").mouseenter()
    }else if($("#gos_reg_num").val() == ""){
        $("#gos_reg_num").focus()
        $("#gos_reg_num").mouseenter()
    }  else if($("#checkButtonImg").attr("class") != "checked"){
        enableblockform()
        $("body").css("cursor","wait");
        $.ajax({
            type: "get",
            async:true,
            url: 'Provider?type=page&id=checkbin&bin='+ $("#bin").val() +"&gos_reg_num="+ $("#gos_reg_num").val(),
            success:function (xml){
                if($(xml).find("valid").text()=="true"){
                    $("#btnsavedoc").removeAttr("disabled").removeClass("ui-state-disabled")
                    $("#checkButtonImg").attr("class", "checked");
                    $("#checkButtonImg").attr("src", "/SharedResources/img/iconset/ok.png")
                    $("[name=orgfullname]").val($(xml).find("orgfullname").text())
                    $("[name=orgfullnamekaz]").val($(xml).find("orgfullnamekaz").text())
                    $("[name=fulllegaladdress]").val($(xml).find("fulllegaladdress").text())
                    $("[name=rukfullname]").val($(xml).find("rukfullname").text())
                }else if($(xml).find("msg").text() != ""){
                    infoDialog($(xml).find("msg").text())
                }else{
                    infoDialog("Ошибка при проверке ИИН/БИН")
                }
...
function enableCheckByIINButton(el){
    if($("#checkButtonImg").attr("class") ==("checked"))  {
