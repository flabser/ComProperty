	...
	<xsl:template match="/request">
		...
		<xsl:for-each select="//query/entry">
			<div style="display:block; width:100%; text-align:left" name="itemStruct">
				...
				<input type="checkbox" name="chbox" value="{replace(viewcontent/viewtext,'&amp;quot;','&#34;')}" id="{@docid}" bin="{viewcontent/viewtext2}" namekaz="{viewcontent/viewtext7}" lgot="{viewcontent/viewtext6}" typeactivity="{viewcontent/viewtext5}" url="{@url}"/>
				<font class="font" title="{@docid}" style="font-family:verdana; font-size:13px; margin-left:2px">
					<xsl:value-of select="replace(viewcontent/viewtext,'&amp;quot;','&#34;')"/>
				</font>
			</div>
		</xsl:for-each>
	</xsl:template>
