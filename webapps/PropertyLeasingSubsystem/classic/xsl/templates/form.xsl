    ...
    <table border="0" style="float:right">
        <tr>
            ...
            <td>
                <font>
                    <xsl:choose>
                        <xsl:when test="document/fields/status='approved'">Подтвержден</xsl:when>
                        <xsl:when test="document/fields/status='rejected'">Отклонен</xsl:when>
                        <xsl:otherwise>Новый</xsl:otherwise>
                    </xsl:choose>
                </font>
            </td>
        </tr>
