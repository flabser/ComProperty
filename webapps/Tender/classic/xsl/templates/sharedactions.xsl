...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="submit_application_for_tender">
        <xsl:if test="document/actionbar/action[@id='SUBMIT_APPLICATION_FOR_TENDER']/@mode = 'ON'">
            <button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:5px">
                <xsl:attribute name="onclick">javascript:window.location.href="Provider?type=edit&amp;id=tenderapplication&amp;key=&amp;parentdocid=<xsl:value-of select="document/fields/tenderid"/>&amp;parentdoctype=<xsl:value-of select="document/@doctype"/>"</xsl:attribute>
                <span>
                    <img src="/SharedResources/img/classic/icons/page_white_add.png" class="button_img"/>
                    <font style="font-size:12px; vertical-align:top"><xsl:value-of select="document/actionbar/action [@id='SUBMIT_APPLICATION_FOR_TENDER']/@caption"/></font>
                </span>
            </button>
        </xsl:if>
    </xsl:template>
