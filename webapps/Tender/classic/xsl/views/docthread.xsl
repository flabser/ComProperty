...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="entry[@form='notification']" mode="line"/>
	<xsl:template match="entry[@form='order']" mode="line"/>	
	<xsl:template match="entry[@form='contract']" mode="line"/>	
	
	<xsl:template match="entry" mode="line">
		<div class="Node" style="overflow:hidden; width:99%; margin-left:10px" id="{@docid}{@doctype}">
	...
	<xsl:template name="graft">
		...
		<xsl:choose>
			<xsl:when  test="following-sibling::entry[@form !='notification'][@form !='order'][@form !='contract']">
	...
	<xsl:template match="*" mode="tree">
		<xsl:choose>
			<xsl:when test="following-sibling::entry[@form !='notification'][@form !='order'][@form !='contract'] and entry[@url]">
