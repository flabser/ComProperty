...
function pickListSingleOk(docid){
	...
	$("#url").val($("#"+docid).attr("url"));
	if($("#rentobjtype").val() !='n'){
		if($("#"+docid).attr("lgot") == '1'){
			if($("#currenttarif").val() != "lgottarif"){
				$("#currenttarif").val("lgottarif")
				$("input[name=tarif]").val($("input[name=tarif]").val() / 2)
			}
		}else{
			if($("#currenttarif").val() == "lgottarif"){
				$("#currenttarif").val("starttarif")
				$("input[name=tarif]").val($("input[name=tarif]").val() * 2)
			}
		}
	}else{
		if($("#"+docid).attr("lgot") == '1'){
			if($("#currenttarif").val() != "lgottarif"){
				$("#currenttarif").val("lgottarif")
				$("input[name=tarif]").val((parseFloat($("#starttarif").val()) / 2) * parseFloat($("#"+docid).attr("typeactivity")))
			}else{
				$("input[name=tarif]").val((parseFloat($("#starttarif").val()) / 2) * parseFloat($("#"+docid).attr("typeactivity")))
			}
		}else{
			if($("#currenttarif").val() == "lgottarif"){
				...
				$("input[name=tarif]").val(parseFloat($("#starttarif").val()) + parseFloat($("#"+docid).attr("typeactivity")))
			}
		}
		
	}
	if($("input[name=id]").val() == 'orderleasing'){
		calcrent()
	}
	if($("input[name=id]").val() == 'rentapplication'){
		$.ajax({
			type: "get",
			url: window.location.protocol + "//" + window.location.host + "/RegistrySubsystem/"+$("#url").val()+"&onlyxml",
			success:function (data){
				$("#rukfullname").val($(data).find("rukfullname").text())
				$("#phone").val($(data).find("phone").text())
				$("#bin").val($(data).find("bin").text())
			}
		});
	}
		...
		if (chBoxes[i].checked && $(chBoxes[i]).attr("id") != ''){ 
			...
			if($("#rentobjtype").val() !='n'){
				...
				if($(chBoxes[i]).attr("lgot") == '1'){
					if($("#currenttarif").val() != "lgottarif"){
						$("#currenttarif").val("lgottarif")
						$("input[name=tarif]").val((parseFloat($("#starttarif").val()) / 2) * parseFloat($(chBoxes[i]).attr("typeactivity")))
					}else{
						$("input[name=tarif]").val((parseFloat($("#starttarif").val()) / 2) * parseFloat($(chBoxes[i]).attr("typeactivity")))
					}
				}else{
					if($("#currenttarif").val() == "lgottarif"){
						...
						$("input[name=tarif]").val(parseFloat($("#starttarif").val()) + parseFloat($(chBoxes[i]).attr("typeactivity")))
					}
				}
			}
		}
...
function dialogBoxStructure(query,isMultiValue, field, form, table) {
	...
	divhtml +="<div  style='display:inline-block ; float:right; width:100px; margin-top:20px'></div></div>";
