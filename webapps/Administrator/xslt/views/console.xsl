			...
			<body style="background-image: url(img/f.gif);">
				<div style="height:600px;">
					<APPLET CODE="kz.pchelka.console.rmi.client.RemoteConsole" CODEBASE="" ARCHIVE="RemoteConsole-signed.jar" WIDTH="90%" HEIGHT="95%">
						<param name="UID">
							<xsl:attribute name="value"><xsl:value-of select="console/@sesid" /></xsl:attribute>
						</param>
						<param name="SERVER" value="NextBase">
							<xsl:attribute name="value"><xsl:value-of select="console/@server" /></xsl:attribute>
						</param>
						<param name="PORT" value="1099">
							<xsl:attribute name="value"><xsl:value-of select="console/@port" /></xsl:attribute>
						</param>
