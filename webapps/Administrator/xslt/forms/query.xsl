...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<head>
			<title>
				Administrator - Query&#xA0; <xsl:value-of select="rule/@id"/>
			</title>
		...
		<body>
			...
			<div style="font-family:verdana; font-size:10px;margin-top:10">
				hits:
				<xsl:value-of select="rule/hits"/>
				<br/>
				turbomode:
				<xsl:value-of select="rule/turbomode"/>
			</div>
			<hr/>
									...
									<tr style="border-bottom:1px solid gray">
										...
										<input type="hidden" name="field">
											<xsl:attribute name="value">
												<xsl:value-of select="name"/>#<xsl:value-of select="publishas"/>#<xsl:value-of select="source"/>^<xsl:value-of select="value"/>^<xsl:value-of select="type"/>
											</xsl:attribute>
										</input>
									</tr>
								</xsl:for-each>
